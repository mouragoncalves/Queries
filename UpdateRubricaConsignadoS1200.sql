...
SELECT *
-- UPDATE wc SET Rubrica = cs.CodigoRubrica
FROM WorkerCredit wc
JOIN ConsignadoS1200 cs ON cs.RelatedYear = wc.[Year] AND cs.RelatedMonth = wc.[Month]
    AND cs.Matricula = wc.Matricula
    AND cs.CPF = wc.CPF
    AND cs.InstituicaoFinanceira = wc.IfConcessoraCodigo
    AND cs.NumeroDocumento = wc.Contrato
    AND cs.ValorRubrica = wc.ValorParcela
